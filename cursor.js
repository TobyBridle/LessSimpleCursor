...
class CursorAttributeList {
    constructor() {
        this.attributes = {
            ...
            "cursor-outline-style": "none",
            "cursor-outline-color": "transparent",
            "cursor-smoothing-position": 0.1,
            "cursor-smoothing-scale": 0.15,
            "cursor-hovers": [],
            "cursor-scroll-snap": true,
        };
    }
}
...
class Cursor extends HTMLElement {
    constructor() {
        ...
        this.cursorAttributes = new CursorAttributeList().attributes;
    ...
    static get observedAttributes() {
        const attributes = Object.keys(new CursorAttributeList().attributes);
        return attributes;
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case "fixed-class":
                ...
                // Inject Custom CSS
                var s = Object.values(document.styleSheets);
            ...
            case "cursor-hovers":
                ...
                s.map((stylesheet) => {
                    try {
                        let sheet = Array.from(stylesheet.cssRules).filter(
                            (rule) =>
                                rule?.selectorText == ".cursor-hover-active"
                        );
                ...
                break;
            case "cursor-scroll-snap":
                if (!!newValue == true)
                    window.addEventListener("scroll", () => {
                        let scrollHorizontal =
                            window.scrollX - this.cursor.scroll.x || 0;
                        let scrollVertical =
                            window.scrollY - this.cursor.scroll.y || 0;

                        this.cursor.previous.x -= scrollHorizontal;
                        this.cursor.previous.y -= scrollVertical;

                        this.cursor.scroll.x = window.scrollX;
                        this.cursor.scroll.y = window.scrollY;
                    });
                break;
        }
